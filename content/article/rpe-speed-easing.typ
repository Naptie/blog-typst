...
#quote[
  ...
  *Added:*
  ...
  - *速度缓动(见注记) √*
  - 自定义预制事件映射(配置文件为./Resources/prefab_mapper.json，默认E键生成一组重置事件) √
  - 可设置自动保存文件数量上限 √
  - 上传至局域网服务器(见注记) √
  - 可从osu和malody的谱面文件(osz&mcz)中导入BPM列表 √
  - 选中单个音符时(单选或多选)，Ctrl+C会将音符的x坐标填入系统剪切板(可选) √
  - 基本信息中显示创建和最后编辑时间 √
  - 剪切板起点时间线染色(可选) √
  ...
  *Adjustments:*
  - 现在曲线填充快捷键(默认Ctrl+F/G)会直接打开曲线填充面板 √

  *注记：*
  1. *约定：速度事件缓动不为1时，实际的速度变化与缓动的导函数形状相同，从而floorposition的变化遵循缓动曲线。为了兼容性，缓动为1时我们保持原含义不变，也即缓动为1和缓动为3都代表二次型的floorposition变化*
  2. 端口始终为8080，上传文件名始终为test.pez。在软件关闭前可在同局域网下的其他设备上下载谱面
  3. RPEVersion已更新至162
]
